...
if (-not $env:QMT_USERDATA_PATH) {
    $qmtDirName = "$([char]0x56FD)$([char]0x91D1)$([char]0x8BC1)$([char]0x5238)QMT$([char]0x4EA4)$([char]0x6613)$([char]0x7AEF)"
...
if (-not (Test-Path -Path $env:QMT_USERDATA_PATH)) {
    ...
    Write-Host "Please check QMT_USERDATA_PATH in start_qmt_bridge_live.ps1"
...
try {
    ...
    Write-Host "[ERROR] QMT Live Bridge failed to start." -ForegroundColor Red
    Write-Host $_
    Write-Host "Checklist:"
    Write-Host "1. QMT live client is running and logged in."
