...
- 对策略研究者：提供策略 DSL、官方策略包、因子库、编译预览、真引擎回测和策略进化实验。
- 对半自动交易用户：提供虚拟仓、实盘仓、实时监控、人工审批、风控熔断和事件追踪。
...
5. 实时监控和执行：支持监控实例、事件流、订单、成交、持仓、审批、熔断和 QMT 桥接。
6. 可观测性：提供 job 状态、SSE 进度、运行日志、数据源治理和结果可信度提示。

### 1.4 当前产品成熟度

从代码结构看，项目已经具备完整 Web 产品骨架和多个可运行业务闭环。策略平台、回测、行情、实时监控、QMT 虚拟仓、资讯之眼、每日复盘和定时任务已具备较高完成度。部分早期路由和新版策略平台并存，说明产品处于快速演进期，仍需要继续统一接口口径、权限边界、文档和配置管理。
...
| 策略研究员 | 创建策略、调整因子、验证收益和风险 | 策略管理、策略回测、因子库、官方策略包 |
| 交易执行用户 | 根据策略信号监控账户并处理订单 | 实时监控、虚拟仓、实盘仓、人工审批 |
...
策略回测用于验证策略在历史数据上的表现。新版策略平台回测支持真引擎、分钟确认、候选池输出、订单、成交、持仓、风险事件、交易快照和大结果分页读取。
...
实时监控将策略、账户、监控池、行情和风控组合起来，支持自动或只读监控模式。每个监控实例会生成事件流，前端通过 SSE 展示状态变化、信号、订单、成交、风控和错误。
...
- 审批列表：`GET /v1/realtime/approvals`
- 审批通过：`POST /v1/realtime/approvals/{approval_id}/approve`
- 审批拒绝：`POST /v1/realtime/approvals/{approval_id}/reject`
...
| 实时监控 | `realtime_monitors`、`realtime_events`、`realtime_approvals` | 监控实例、事件、审批 |
...
2. 实盘仓能力需要更强风控边界，包括账户白名单、订单上限、审批策略和审计日志。
3. 旧策略路由和新版策略平台并存，用户和开发者可能混淆。
...
6. 控制台增加今日待处理事项：审批、失败任务、数据缺口、待复盘报告。
...
让用户在不同页面看到同一个概念时，理解一致、命名一致、状态一致、操作结果一致。当前项目已经覆盖智能分析、策略管理、策略回测、实时监控、虚拟仓、实盘仓、每日复盘和数据治理等多个域，如果术语、状态和入口继续分散，用户会把强功能理解成“很多互相不完全对齐的模块”。产品口径统一的目标是让所有模块形成同一套语言和同一条业务链路。

#### 16.1.2 需要统一的核心口径

| 口径对象 | 当前风险 | 统一方向 |
|---|---|---|
| 策略 | 旧策略接口、新策略平台、官方策略包、策略版本可能并存 | 统一称为“策略”，明确草稿、运行中、暂停、归档、官方示例、克隆策略、版本策略 |
...
| 策略研究 | 策略管理、策略回测、回测结果、策略进化 | 我如何创建、验证和迭代策略 |
| 交易监控 | 实时监控、虚拟仓、实盘仓、审批任务 | 我如何把信号安全地接到交易 |
...
2. 路由层逐步收敛：
   ...
   - 老版 `/api/v1/strategies` 和 `/api/v1/backtest` 保留兼容期，但在文档中标注 legacy。
...
| P0 | 标记 legacy 接口和主接口 | API 文档、前端 API 注释 |
...
- 用户在策略管理、回测、实时监控中看到的同一策略名称、状态和版本一致。
- 所有核心页面状态标签来自同一映射表。
...
1. 默认只读：任何实盘账户默认只能查看，不能自动下单。
2. 显式授权：启用实盘交易必须由用户明确确认。
...
| 订单 | `order.requires_approval` | true | 默认走人工审批 |
...
| `require_manual_approval` | 默认人工审批 | true |
...
| 场景 | 页面提示 |
|---|---|
| 实盘未授权 | “当前为只读实盘视图，不会提交任何委托。” |
| 实盘已授权但需审批 | “策略信号将生成审批任务，人工确认后才会下单。” |
