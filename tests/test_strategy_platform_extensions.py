...
def test_daily_kline_parquet_loader_supports_column_projection(tmp_path: Path):
    frame = pd.DataFrame(
        [
            {
                "symbol": "000001.SZ",
    ...
    assert loaded is not None
    assert list(loaded.columns) == ["symbol", "date", "close", "amount"]
...
def test_export_daily_kline_to_parquet_from_database(tmp_path: Path, monkeypatch):
    ...
    with isolated_postgres_engine(schema_prefix="ta_daily_export") as (engine, database_url, _schema):
        frame.to_sql("stock_daily_kline", engine, index=False, dtype={"trade_date": Date()})
