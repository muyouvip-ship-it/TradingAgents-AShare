...
@pytest.fixture
def db():
    with isolated_postgres_session(Base, schema_prefix="ta_report_recovery") as session:
        yield session
...
def test_recover_stale_active_reports_marks_empty_running_report_failed(db):
...
def test_recover_stale_active_reports_marks_partial_running_report_failed(db):
    report = _add_report(
        db,
        status="running",
...
def test_finalize_orphan_report_marks_pending_report_failed(db):
