...
const defaultPrompt = '创建一个选股策略：算力板块、流通市值 100 亿到 200 亿之间、业绩暴增。'

const strategyTypeOptions: Array<{ value: StrategyPlatformType; label: string; description: string }> = [
  { value: 'selection', label: '选股策略', description: '只定义股票池、因子和筛选逻辑。' },
  { value: 'trading', label: '交易策略', description: '重点定义买入、卖出与信号规则。' },
  { value: 'risk', label: '风控策略', description: '重点定义仓位与风险约束。' },
  { value: 'portfolio', label: '组合策略', description: '选股、交易、仓位、风控一体化。' },
]

const modeOptions: Array<{ value: CreateMode; label: string; description: string }> = [
  { value: 'dsl', label: 'DSL 直编', description: '直接编写或粘贴 DSL，先校验再保存。' },
  { value: 'llm', label: '白话生成', description: '先用白话生成初稿，再在 DSL 编辑器里确认与修改。' },
]
...
const baseDslTemplate: StrategyDsl = {
  ...
  factor_model: {
    ...
    factors: [
      { name: 'net_profit_growth_yoy', weight: 0.4, direction: 'higher_better', transform: 'rank_pct' },
...
export default function StrategyCreate() {
  ...
  const [selectedStrategyType, setSelectedStrategyType] = useState<StrategyPlatformType>('selection')
  const [prompt, setPrompt] = useState(defaultPrompt)
  const [existingStrategy, setExistingStrategy] = useState<StrategyDefinition | null>(null)
  const [draftMeta, setDraftMeta] = useState<StrategyDraftResponse | null>(null)
  const [manualName, setManualName] = useState('我的策略')
  const [manualDescription, setManualDescription] = useState('只保存策略定义；回测参数在回测页单独配置。')
  const [dslEditorText, setDslEditorText] = useState(() => JSON.stringify(localDraftFallback.dsl, null, 2))
  const [dslError, setDslError] = useState<string | null>(null)
  const [compileReport, setCompileReport] = useState<StrategyCompileResponse | null>(null)
  ...
  useEffect(() => {
    ...
    const loadExistingStrategy = async () => {
      ...
      try {
        ...
        const dsl = normalizeDslForStrategyType(strategy.current_version?.dsl ?? buildBaseDsl(strategy.strategy_type), strategy.strategy_type)
        ...
        setManualDescription(strategy.description || '只保存策略定义；回测参数在回测页单独配置。')
  ...
  const handleStrategyTypeChange = (nextType: StrategyPlatformType) => {
    ...
    setLastValidatedText(null)
    setMessage(null)
    setDraftMeta(current => current ? { ...current, strategy_type: nextType, dsl: normalizeDslForStrategyType(current.dsl, nextType) } : current)
    setDslEditorText(current => {
      const parsed = parseStrategyDsl(current)
      const nextDsl = normalizeDslForStrategyType(parsed ?? buildBaseDsl(nextType), nextType)
  ...
  const generateDraft = async () => {
    ...
    setMessage(null)
    try {
      const response = await api.createStrategyDraft(prompt, selectedStrategyType)
      const normalizedDsl = normalizeDslForStrategyType(response.dsl, response.strategy_type)
      ...
      setLastValidatedText(response.compile_report?.status === 'passed' ? formattedDsl : null)
      setSchemaLoaded(Boolean(response.structured_output_schema) || schemaLoaded)
      setMessage('已生成 DSL 草稿；请继续校验并保存。')
    } catch (error) {
      console.warn('LLM 草案接口暂不可用，使用本地草案预览', error)
      const fallback = buildFallbackMeta(selectedStrategyType)
      const formattedDsl = JSON.stringify(fallback.dsl, null, 2)
      setDraftMeta(fallback)
      setManualName(fallback.name)
      setManualDescription(fallback.intent_summary)
      setDslEditorText(formattedDsl)
      setDslError(null)
      setCompileReport(null)
      setLastValidatedText(null)
      setMessage('后端暂不可用，当前显示本地 DSL 草案。')
    } finally {
      setLoadingDraft(false)
    }
  }

  const validateDsl = async (): Promise<{ dsl: StrategyDsl; report: StrategyCompileResponse; formattedText: string } | null> => {
    ...
    const normalizedDsl = normalizeDslForStrategyType(parsed, selectedStrategyType)
    ...
    try {
      ...
      const localReport = buildLocalCompileReport(normalizedDsl, error)
      setCompileReport(localReport)
      if (localReport.status === 'passed') {
        setLastValidatedText(formattedText)
        setMessage('后端预校验暂不可用，已完成前端本地校验；建议后端恢复后再跑一次完整编译。')
      } else {
        setLastValidatedText(null)
        setMessage('DSL 本地校验未通过，请先修复报错。')
      }
      return { dsl: normalizedDsl, report: localReport, formattedText }
    } finally {
      setValidating(false)
    }
  }
  ...
  const ensureValidatedDsl = async () => {
    if (compileReport?.status === 'passed' && lastValidatedText === dslEditorText) {
      const parsed = parseStrategyDsl(dslEditorText)
  ...
  const saveStrategy = async (goBacktest: boolean) => {
    ...
    try {
      const payload = {
        name: manualName.trim() || `未命名${toChineseStrategyType(selectedStrategyType)}`,
        strategy_type: selectedStrategyType,
        description: manualDescription.trim() || '仅保存策略定义；回测参数请在回测页配置。',
        source: existingStrategy?.source ?? (createMode === 'llm' ? 'llm' : 'manual'),
      ...
      setMessage(strategyId ? '策略修改已保存，并生成新版本。' : '策略已保存。')
      if (goBacktest) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-600 dark:bg-blue-500/10 dark:text-blue-300">
              ...
              {isEditMode ? '编辑策略定义，回测参数仍在回测环节配置' : '只创建策略，回测参数移到回测环节'}
            </div>
            <h1 className="mt-3 text-2xl font-bold text-slate-900 dark:text-slate-100">{isEditMode ? '策略编辑器' : '策略创建器'}</h1>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {isEditMode ? '修改策略名称、说明和 DSL；保存后会生成一个新的当前版本。' : '以 DSL 编辑器为主，支持白话生成初稿与 DSL 直编；保存前先做 DSL 校验。'}
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        <div className="space-y-4">
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">创建方式</h2>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">策略类型</h2>
            <div className="mt-4 grid gap-3">
              {strategyTypeOptions.map(option => {
                ...
                <div className="text-sm font-semibold">{option.label}</div>
                    <div className="mt-2 text-sm opacity-80">{option.description}</div>
                  </button>
                )
              })}
            </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">策略信息</h2>
            <div className="mt-4 grid gap-4">
              <label className="block">
                <div className="text-sm font-medium text-slate-700 dark:text-slate-200">策略名称</div>
                <input
                  value={manualName}
              ...
              <label className="block">
                <div className="text-sm font-medium text-slate-700 dark:text-slate-200">策略说明</div>
                <textarea
          ...
          {createMode === 'llm' && (
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">白话生成只负责产出 DSL 草稿，最终保存内容以当前编辑器中的 DSL 为准。</p>
              <textarea
                value={prompt}
              ...
              >
                <SparklesIcon className={`h-4 w-4 ${loadingDraft ? 'animate-pulse' : ''}`} />
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">创建说明</h2>
            <div className="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-300">
              <InfoRow label="当前模式" value={createMode === 'llm' ? '白话生成 + DSL 编辑' : 'DSL 直编'} />
              <InfoRow label="策略类型" value={toChineseStrategyType(selectedStrategyType)} />
              <InfoRow label="Schema 约束" value={schemaLoaded ? '已加载 Pydantic JSON Schema' : '本地约束预览'} />
              <InfoRow label="页面职责" value={isEditMode ? '仅编辑策略定义，不在此页配置回测参数。' : '仅创建策略定义，不在此页配置回测参数。'} />
              {isEditMode && existingStrategy ? <InfoRow label="当前版本" value={`v${existingStrategy.version}`} /> : null}
            </div>
          </section>
        ...
        <div className="space-y-4">
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">策略类型切换会同步修正 `strategy_type`；保存前必须先通过 DSL 校验。</p>
              </div>
            ...
            <textarea
              ...
              onChange={event => {
                ...
                setLastValidatedText(null)
                setMessage(null)
                setDslError(parseStrategyDsl(event.target.value) ? null : 'DSL JSON 解析失败，请检查括号、逗号和字段格式。')
              }}
              rows={30}
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">DSL 校验结果</h2>
            {compileReport ? (
              <>
                <div className="mt-4 grid gap-3 md:grid-cols-3">
                  <MetricCard label="校验状态" value={toChineseCompileStatus(compileReport.status)} />
                  <MetricCard label="因子数量" value={String(compileReport.factor_count ?? 0)} />
                  <MetricCard label="入场 / 出场" value={`${compileReport.entry_rule_count ?? 0} / ${compileReport.exit_rule_count ?? 0}`} />
                </div>
                <div className="mt-4 grid gap-3 md:grid-cols-3">
                  <MetricCard label="时间周期" value={(compileReport.timeframes_required || []).join('、') || '--'} />
                  <MetricCard label="分钟要求" value={formatMinuteRequirement(compileReport.minute_requirements)} />
                  <MetricCard label="执行后端" value={formatBackendResolution(compileReport.backend_resolution)} />
                </div>
                {!!compileReport.errors.length && (
                  <div className="mt-4 space-y-2">
                    {compileReport.errors.map(item => (
                      <div key={item} className="rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-500/10 dark:text-rose-200">
                        {item}
                      </div>
                    ))}
                  </div>
                ...
                {!!compileReport.warnings.length && (
                  <div className="mt-4 space-y-2">
                    {compileReport.warnings.map(item => (
                      <div key={item} className="rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:bg-amber-500/10 dark:text-amber-200">
                        {item}
                      </div>
                    ))}
                  </div>
...
function InfoListCard({
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <h2 className="flex items-center gap-2 text-lg font-semibold text-slate-900 dark:text-slate-100">
        {title === '待确认项' ? <ExclamationTriangleIcon className="h-5 w-5 text-amber-500" /> : <CheckCircleIcon className="h-5 w-5 text-emerald-500" />}
        {title}
      </h2>
      <div className="mt-4 space-y-2">
        {confirmations?.length ? confirmations.map(item => (
          <div key={`${item.field}-${item.assumed_as}`} className="rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:bg-amber-500/10 dark:text-amber-200">
            <div className="font-medium">{item.field} → {item.assumed_as}</div>
            <div className="mt-1">{item.reason}</div>
          </div>
        )) : null}
        {!confirmations?.length && items?.length ? items.map(item => (
          <div key={item} className={`rounded-xl px-3 py-2 text-sm ${toneClass}`}>
            {item}
          </div>
        )) : null}
...
function buildFallbackMeta(strategyType: StrategyPlatformType): StrategyDraftResponse {
  const dsl = normalizeDslForStrategyType(baseDslTemplate, strategyType)
  return {
    ...localDraftFallback,
    name: strategyType === 'selection' ? '自定义选股策略' : `自定义${toChineseStrategyType(strategyType)}`,
...
function normalizeDslForStrategyType(dsl: StrategyDsl, strategyType: StrategyPlatformType): StrategyDsl {
...
function buildLocalCompileReport(dsl: StrategyDsl, error?: unknown): StrategyCompileResponse {
  const errors: string[] = []
  const warnings: string[] = []
  const requiredFields = new Set<string>(['open', 'close'])
  const compiledTargets = ['frontend_json_parse', 'frontend_structural_check']
  const timeframes = new Set<string>(['1d'])
  const factors = Array.isArray(dsl.factor_model?.factors) ? dsl.factor_model.factors as Array<Record<string, unknown>> : []
  const entryConditions = Array.isArray(dsl.entry?.conditions) ? dsl.entry.conditions as Array<Record<string, unknown>> : []
  const exitConditions = Array.isArray(dsl.exit?.conditions) ? dsl.exit.conditions as Array<Record<string, unknown>> : []
  const strategyTypes = new Set(['selection', 'trading', 'risk', 'portfolio'])

  if (!dsl.schema_version) errors.push('schema_version 不能为空。')
  if (!strategyTypes.has(dsl.strategy_type)) errors.push('strategy_type 必须是 selection / trading / risk / portfolio。')
  if (!factors.length) errors.push('factor_model.factors 不能为空。')

  const select = (dsl.factor_model?.select || {}) as Record<string, unknown>
  const minScore = Number(select.min_score ?? 0.6)
  if (Number.isNaN(minScore) || minScore < 0 || minScore > 1) {
    errors.push('factor_model.select.min_score 必须在 0 到 1 之间。')
  }

  for (const factor of factors) {
    const name = String(factor.name || '')
    if (!name) {
      errors.push('factor_model.factors 中存在未命名因子。')
      continue
    }
  ...
  for (const condition of [...entryConditions, ...exitConditions]) {
    const timeframe = String(condition.timeframe || '1d')
    timeframes.add(timeframe)
    if (timeframe.endsWith('m')) {
      requiredFields.add(`minute_${timeframe}.close`)
    }
  }

  const execution = dsl.execution || {}
  if (Number(execution.lot_size || 100) !== 100) {
    warnings.push('A 股默认按 100 股整数手撮合，当前 DSL 已偏离默认设置。')
  }
  const minuteLoading = (execution.minute_loading || {}) as Record<string, unknown>
  if (minuteLoading.forbid_full_market_preload !== true) {
    warnings.push('建议明确禁止全市场分钟线预加载，避免分钟级 OOM。')
  }
  if (error instanceof Error) {
    warnings.push(`后端预校验不可用：${error.message}`)
  }

  const minuteTimeframes = Array.from(timeframes).filter(item => item.endsWith('m')).sort()
  return {
    status: errors.length ? 'failed' : 'passed',
    errors,
    warnings,
    required_fields: Array.from(requiredFields).sort(),
    compiled_targets: compiledTargets,
    factor_count: factors.length,
    entry_rule_count: entryConditions.length,
    exit_rule_count: exitConditions.length,
    runtime_engine: {
      scan: 'frontend_local_check',
      compute: 'frontend_local_check',
      minute_loading: minuteLoading.mode || 'lazy_by_watchlist',
    },
    execution_plan: {
      selection_plan: {
        market: (dsl.universe?.market || 'A_SHARE') as string,
        filters: dsl.universe?.filters || [],
        include_concepts: dsl.universe?.include_concepts || [],
        score_method: dsl.factor_model?.score_method || 'weighted_sum',
        top_n: Number(select.top_n || 30),
        min_score: minScore,
      },
      entry_rule_plan: entryConditions,
      exit_rule_plan: exitConditions,
    },
    timeframes_required: Array.from(timeframes).sort(),
    minute_requirements: {
      enabled: minuteTimeframes.length > 0,
      timeframes: minuteTimeframes,
      source_table: minuteTimeframes.length ? 'stock_minute_kline' : null,
      loading_mode: minuteLoading.mode || 'lazy_by_watchlist',
    },
    backend_resolution: {
      scan: 'frontend_local_check',
      compute: 'frontend_local_check',
...
function formatMinuteRequirement(value?: Record<string, unknown>) {
  ...
  const timeframes = Array.isArray(value.timeframes) ? value.timeframes.join('、') : '已启用'
