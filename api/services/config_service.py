...
def config_response_for_user(user: Optional[UserDB], db: Session) -> UserRuntimeConfigResponse:
    ...
    return UserRuntimeConfigResponse(
        ...
        news_llm_provider=str(getattr(user_cfg, "news_llm_provider", None) or ""),
        news_backend_url=str(getattr(user_cfg, "news_backend_url", None) or ""),
        news_analysis_llm=str(getattr(user_cfg, "news_analysis_llm", None) or ""),
